...
var SecureBuffer = dejavu.Class.declare({
    ...
    getRaw: function getRaw(acknowledgeDanger) {
        if(acknowledgeDanger !== 'ACKNOWLEDGE-DANGER') {
            throw new Error('You must pass "ACKNOWLEDGE-DANGER" to SecureBuffer.getRaw() to acknowledge the danger of directly accessing a SecureBuffer.')
        }
        return this.__instance;
    },
    ...
    copy: function copy() {
        console.warn('[SecureBuffer] Warning, you should NOT use copy() on a SecureBuffer instance.');
        return this.__instance.copy.apply(this.__instance, arguments);
    },
    
    slice: function slice(start, end) {
        if(!start) start = 0;
        if(!end) end = this.__instance.length;
        
        var slicedBuffer = new Buffer(end - start);
        this.__instance.copy(slicedBuffer, 0, start, end);
        
        return new SecureBuffer(slicedBuffer);;
    },
