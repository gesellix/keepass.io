...
var GroupObject = dejavu.Class.declare({
    ...
    __parseGroups: function(payload) {
        if(!payload.Group) return null;
        var groups = Array.isArray(payload.Group) ? payload.Group : [payload.Group];
        
        for(var key in groups) {
            this.__groups[groups[key].UUID] = new GroupApi(new GroupObject(groups[key]));
        }
    },
    
    __parseEntries: function(payload) {
        if(!payload.Entry) return null;
        var entries = Array.isArray(payload.Entry) ? payload.Entry : [payload.Entry];
        
        for(var key in entries) {
            this.__entries[entries[key].UUID] = new EntryApi(new EntryObject(entries[key]));
        }
    },
    ...
    getEntries: function() { return this.__entries; }
});
