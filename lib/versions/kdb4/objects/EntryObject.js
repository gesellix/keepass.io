...
var EntryObject = dejavu.Class.declare({
    ...
    __unpackStrings: function __unpackStrings(payload) {
    ...
    __unpackHistoryEntries: function __unpackHistoryEntries(payload) {
        ...
        for(var key in entries) {
            this.getParent().getUuidManager().markUsed(entries[key].UUID);
            this.__history.push(new EntryApi(new EntryObject(entries[key], this.getParent())));
        }
    },
    ...
    handleStringsCall: function handleStringCall(parent, key, value) {
        if(key === undefined) {
    ...
    handleHistoryCall: function handleHistoryCall() {
